...
public class AccountsController {
 ...
 //   log.info("Creating account {}", account);
  ...
  public String transferMoney(@RequestBody @Valid MoneyTransferRequest moneyTransferRequest) throws Exception {
	  String result="";
		try {
		result = this.accountsService.transferMoney(moneyTransferRequest);
		
		
		log.info("TransferMoner res:"+result) ;			
			
		} catch (AccountNotExistException | OverDraftException e) {
			log.error("Money Transfer Failed, check logs for more details");
			throw e;
		} catch (Exception ee) {
			log.error("Exception Occured, check logs for more details");
			throw ee;
		}
		return result;
	}
